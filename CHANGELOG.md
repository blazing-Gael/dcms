...
- **Built-in admin panel at `/__admin` (ADR-0035, phase 1).** DCMS was API-only, so
  ...
  **privileged client, not a backdoor**: every write runs through the same authorized
  ...
  It also has an admin-role-gated **System** section: full **user management**
  (create/edit, role assignment, active/disabled status, password reset, sign-out-
  everywhere, delete — reusing the API's guarded primitives, including the
  last-active-admin guard so the panel can't lock the instance out of itself); an
  **access map** (a read-only, schema-derived matrix of who may read/create/update/
  delete/preview/publish per collection, and read/write per field); **read-only
  system views** over the engine-managed collections the API hides (the change feed,
...
### Added
- **Long-lived, revocable API tokens for machine callers (issue #8).** A
  non-human caller — an SSG build step, a CI job, a webhook receiver reading back
  the changed record — no longer has to impersonate a user with a password in its
  env. `dcms token create --name ci --role reader --expires 90d` mints a token
