...
| [0032](./0032-wasm-plugin-transport.md) | WASM plugin transport: the single dropped-in-a-folder extension method — wazero (pure-Go), JSON-over-memory ABI implementing the ADR-0031 contract, sandboxed + fuel/memory-limited, polyglot; guest SDKs (Rust/TinyGo) + `dcms plugin new` + typed bindings from schema as scoped DX; no embedded interpreter, WIT/marketplace deferred | Proposed |
