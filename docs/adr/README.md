...
| [0022](./0022-core-vs-plugin-boundary.md) | Core vs. seam vs. out-of-scope: a four-bucket test for what ships in core, what is a swappable seam with a default, and what is the app's job via events; settles general email (app's job) and OIDC (generic verifier is opt-in core, after issue #9 wiring) | Accepted |
| [0023](./0023-preview-access-rule.md) | Identity-based preview: a per-record `preview` access rule for hidden lifecycle states (draft/scheduled/archived/trashed), opt-in per collection and the single source of truth for read+write visibility when set; the shared token stays for machines/links (issue #20) | Accepted |
| [0024](./0024-publish-rule-and-revision-preview.md) | A `publish` access rule gating go-live transitions separately from `update` (opt-in, falls back to `update`); revision history follows the `preview` rule per record instead of the shared token (issues #23, #24) | Accepted |
| [0025](./0025-strict-config-and-schema-keys.md) | Strict config & schema keys: an unknown key is a load/compile error (with did-you-mean); reserved future directives warn; a hard break over a warn-then-error rollout (issue #35) | Accepted |
| [0026](./0026-scheduled-golive-events.md) | Scheduled go-lives emit a `went_live` event via a due-at outbox worker, so the change feed/webhooks are complete; core gains exactly one time trigger (not a general scheduler) — issue #28 | Accepted |
| [0027](./0027-optimistic-concurrency.md) | Optimistic concurrency: opt-in `concurrency` adds an engine-managed `_version` counter + `If-Match` → 412 on a stale write; gateway-owned, per-request opt-in, no silent no-op (issue #26) | Accepted |
| [0028](./0028-media-access-inheritance.md) | Media access by inheritance: `_media.access.read: inherit` makes a file readable iff the caller can read a record that references it (else the uploader), so shared and private files coexist on one library (issue #30) | Accepted |
| [0029](./0029-passwordless-email-otp-login.md) | Passwordless email-OTP login: opt-in `auth.otp` mints a short single-use code (reusing `_auth_tokens` + the notification outbox), exchanged for a normal session; short TTL + attempt cap + invalidate-on-reissue + per-email throttle (issue #11) | Accepted |
| [0030](./0030-object-list-field-type.md) | `object_list` field type: repeatable groups of fields with an inline `of` shape, stored as JSON by reusing `FieldDef` recursively one level deep; per-element validation, batched inner-reference checks, array-of-object codegen (issue #6) | Accepted |
| [0031](./0031-extension-hooks.md) | Extension hooks: opt-in in-process Go hooks on the write lifecycle of generated endpoints (+ custom routes), Tx-participating and identity-safe, handed a narrow store interface; values-not-shape guardrail; RPC/WASM transports and a marketplace explicitly deferred | Accepted |
| [0034](./0034-value-scoped-write-rules.md) | Value-scoped write rules: a declarative `write: { rules: [{who, from, to}] }` transition table on enum fields (issue #27); core not hook because it's authz + part of the contract; disallowed transition ⇒ 403 naming the field, no applicable rule ⇒ silent drop, no-op always allowed, create gated by who+to | Accepted |
| [0035](./0035-admin-panel.md) | Admin panel: an embedded, server-rendered (`html/template` + `//go:embed`, htmx for dynamic bits, no SPA/build) window into the instance at `/__admin`, generated from the schema; observability-first (collection browser + system views + read-only data-model viz), CRUD reuses the authorized write pipeline (privileged client, not a backdoor; CSRF on forms), schema is read-only, config-driven + custom pages | Accepted |
