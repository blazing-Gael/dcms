...
| [0033](./0033-compile-and-ship-extensions.md) | Compile-and-ship is the primary extension model: build a custom binary with hooks via the library (ADR-0031) and ship it; a `dcms new` scaffold + CI/Docker recover the `curl`/pull convenience; dev==prod parity, native speed, no runtime plugin machinery. WASM (ADR-0032) deferred to a conditional need (zero-build / polyglot / untrusted plugins) | Accepted |
