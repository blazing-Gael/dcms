...
// dsn appends the pragmas we require on every connection to the SQLite path.
//
// This matters for correctness, not just tuning: PRAGMA foreign_keys is a
// per-connection setting, so running it once on the pool would leave it OFF on
// every other pooled connection and silently disable referential integrity
// (ADR-0010). modernc.org/sqlite applies `_pragma` DSN parameters as each
// connection opens, so putting them here guarantees they hold pool-wide.
//   - foreign_keys(1) — enforce relations on every connection.
//   - busy_timeout(5000) — wait, don't error, when the DB is briefly locked.
...
func dsn(path string) string {
	...
	return path + sep + "_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)"
}
...
func (c *conn) Delete(ctx context.Context, collection, id string) error {
	...
	if err != nil {
		// A RESTRICT foreign key blocks deleting a still-referenced row; map it to
		// ErrIntegrity so the gateway can answer 409 (ADR-0010).
...
func (c *conn) Introspect(ctx context.Context, collection string) (store.CollectionMeta, error) {
	...
	// Foreign keys, so Diff can compare against the desired FKs (ADR-0010) and
	// refuse to silently drift when one is added/changed/dropped on an existing
	// column. PRAGMA foreign_key_list gives one row per FK: the local column
	// ("from"), the referenced table, and the ON DELETE action.
	fkRecs, err := c.queryRecords(ctx, fmt.Sprintf("PRAGMA foreign_key_list(%s);", quote(collection)))
	if err != nil {
		return store.CollectionMeta{}, err
	}
	for _, fk := range fkRecs {
		from, _ := fk["from"].(string)
		i, ok := colIdx[from]
		if !ok {
			continue
		}
		refTable, _ := fk["table"].(string)
		meta.Columns[i].References = refTable
		meta.Columns[i].OnDelete = onDeleteFromPragma(fk["on_delete"])
	}
...
func (c *conn) Diff(ctx context.Context, desired store.CollectionMeta) (store.MigrationPlan, error) {
	...
	haveCol := make(map[string]store.ColumnMeta, len(current.Columns))
	for _, col := range current.Columns {
		haveCol[col.Name] = col
	}
	...
	for _, col := range desired.Columns {
		if cur, ok := haveCol[col.Name]; ok {
			// The column exists. The additive migrator never retypes columns, but
			// a foreign key that was added, changed, or dropped can't be silently
			// ignored — SQLite can only change an FK via a full table rebuild,
			// which we don't do yet (ADR-0010). Detect it and fail with guidance
			// rather than drift or a cryptic error later.
			if !fkEqual(cur, col) && (col.References != "" || cur.References != "") {
				return store.MigrationPlan{}, fmt.Errorf(
					"%w: %s.%s: changing a foreign key on an existing column requires a table rebuild (not yet supported). Recreate the collection, or add the relation to a new collection",
					store.ErrInvalidInput, desired.Name, col.Name)
			}
			continue
		}
		// Adding a NOT NULL column with no default to an existing table fails on
		// SQLite once the table has rows. This is exactly what retrofitting a
		// *required* relation onto an existing collection would do — surface a
		// clear error instead of a raw SQLite failure. (A nullable relation, or
		// one with a default, adds fine — including its REFERENCES clause.)
		if !col.Nullable && col.Default == nil {
			return store.MigrationPlan{}, fmt.Errorf(
				"%w: %s.%s: cannot add a required column to an existing table (SQLite rejects a NOT NULL column without a default on populated data). Make it nullable, give it a default, or recreate the collection",
				store.ErrInvalidInput, desired.Name, col.Name)
		}
