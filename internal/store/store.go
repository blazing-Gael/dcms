...
type ColumnMeta struct {
	...
	// References, when non-empty, marks this column as a foreign key pointing at
	// References's id column. OnDelete is the referential action on parent delete
	// ("cascade", "set null", or "" / "restrict" for the default). Adapters that
	// support foreign keys emit them at CREATE TABLE; adapters that don't ignore
	// these fields. Referential integrity is validated at the gateway regardless
	// (see ADR-0010) — these drive the database-level protection layer only.
	References string
	OnDelete   string
}
